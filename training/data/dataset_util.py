...
def _require_cv2():
    ...
    if cv2 is None:
        try:
            ...
            raise ImportError(
                "OpenCV is required for this dataset operation. "
                "Install `opencv-python` that matches the current NumPy version."
            ) from exc
    return cv2
...
def resize_image_depth_and_intrinsic(
    ...
    if depth_map is not None:
        ...
        if cv2_mod is not None:
            ...
            depth_map = np.array(
                Image.fromarray(depth_map.astype(np.float32), mode="F").resize(
                    tuple(output_resolution), resample=Image.NEAREST
                ),
                dtype=np.float32,
            )
...
def read_image_cv2(path: str, rgb: bool = True) -> np.ndarray:
    ...
    if rgb:
        if len(img.shape) == 2:
            img = cv2_mod.cvtColor(img, cv2_mod.COLOR_GRAY2BGR)
        else:
...
def read_depth(path: str, scale_adjustment=1.0) -> np.ndarray:
    ...
    if path.lower().endswith(".exr"):
        ...
        d = cv2_mod.imread(path, cv2_mod.IMREAD_ANYCOLOR | cv2_mod.IMREAD_ANYDEPTH)[..., 0]
        d[d > 1e9] = 0.0
